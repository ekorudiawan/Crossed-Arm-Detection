...
cap = cv.VideoCapture('./videos/bob2-right.avi')
...
while True:
    ...
    if ret:
        ...
        # If right hand keypoint already in left region or left hand keypoint already in right region
        if len(lhcross_idx[0]) > 0 or len(rhcross_idx[0]):
            detecting = True
